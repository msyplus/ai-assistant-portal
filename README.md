...
这是一个基于真实服务运营经验构建的 AI Demo 作品集门户，聚合 6 个 Streamlit 应用：VOC 智能分类、批量异常预警、客服对话质检、客服 SOP 知识库问答、服务事件智能摘要和 AI 助手门户。其中 5 个为业务能力 Demo，1 个为统一门户。

作品集的核心不是展示单点工具，而是展示一条服务 AI 工作流：

```text
识别问题 → 判断风险 → 评估质量 → 检索知识 → 沉淀信息 → 统一入口
```
...
这 6 个 Demo 不是随机练手项目，而是我把过去做过的服务 AI 系统重新拆成六个能力模块：先识别问题是什么，再判断风险有多高，再评估服务质量，再检索 SOP 知识依据，再生成结构化摘要，最后统一入口降低工具切换成本。这对应我在携程做 AI 项目时的核心理解：AI 不应该成为新的工具负担，而应该嵌入工作流。
